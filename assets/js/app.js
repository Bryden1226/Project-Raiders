...
function renderHome(){
  ...
  const total=wb.entries+ex.entries+bpTotal;
  const done=wb.done+ex.done+bpDone;
  ...
  $('#overallDetail').textContent=`${done} of ${total} tracked goals complete`;
  $('#upPct').textContent=wbPct+'%';
  $('#upStat').textContent=`${wb.done} / ${wb.entries} material entries`;
  $('#upHomeBar').style.width=wbPct+'%';
  $('#exPct').textContent=exPct+'%';
  $('#exStat').textContent=`${ex.done} / ${ex.entries} material entries`;
  ...
  blueprints.forEach(b=>{
    categoryCounts[b.category]??={done:0,total:0};
    categoryCounts[b.category].total++;
    if(state.blueprints[b.name]) categoryCounts[b.category].done++;
  });
  $('#blueprintHomeBreakdown').innerHTML=Object.entries(categoryCounts).sort((a,b)=>a[0].localeCompare(b[0])).map(([category,v])=>{
    const pct=v.total?Math.round(v.done/v.total*100):0;
    return `<div class="home-progress-row"><div><span>${esc(category)}</span><b>${v.done}/${v.total}</b></div><div class="mini-progress"><span style="width:${pct}%"></span></div></div>`;
  }).join('');
