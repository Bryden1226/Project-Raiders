...
let activeMapId=maps[0]?.id||''; let activeMapType='All';
const mapThemeArt={
  wetlands:'radial-gradient(circle at 22% 72%,rgba(70,110,91,.55),transparent 24%),radial-gradient(circle at 65% 35%,rgba(74,105,109,.38),transparent 28%),linear-gradient(145deg,#17272b,#0b161a 58%,#1c2520)',
  desert:'radial-gradient(circle at 55% 35%,rgba(170,126,70,.32),transparent 25%),radial-gradient(circle at 26% 65%,rgba(120,84,50,.28),transparent 26%),linear-gradient(145deg,#31291e,#151713 60%,#242014)',
  city:'linear-gradient(90deg,transparent 48%,rgba(255,255,255,.035) 49%,rgba(255,255,255,.035) 51%,transparent 52%),linear-gradient(0deg,transparent 48%,rgba(255,255,255,.035) 49%,rgba(255,255,255,.035) 51%,transparent 52%),linear-gradient(145deg,#29231d,#121618)',
  mountain:'radial-gradient(ellipse at 25% 25%,rgba(111,128,119,.3),transparent 25%),radial-gradient(ellipse at 78% 68%,rgba(92,110,101,.3),transparent 26%),linear-gradient(145deg,#27302c,#111719)',
  facility:'linear-gradient(90deg,transparent 48%,rgba(255,255,255,.04) 49%,rgba(255,255,255,.04) 51%,transparent 52%),linear-gradient(0deg,transparent 48%,rgba(255,255,255,.04) 49%,rgba(255,255,255,.04) 51%,transparent 52%),linear-gradient(145deg,#20282d,#0d1317)',
  coast:'radial-gradient(ellipse at 78% 58%,rgba(48,112,137,.35),transparent 34%),linear-gradient(120deg,#2d3127 0 48%,#10222b 49% 100%)'
};
function renderMaps(){
  if(!$('#mapSelect')) return;
  $('#mapSelect').innerHTML=maps.map(m=>`<option value="${esc(m.id)}" ${m.id===activeMapId?'selected':''}>${esc(m.name)}</option>`).join('');
  const map=maps.find(m=>m.id===activeMapId)||maps[0]; if(!map)return;
  const types=['All',...new Set(map.zones.map(z=>z.type))];
  if(!types.includes(activeMapType)) activeMapType='All';
  $('#mapTypeFilters').innerHTML=types.map(t=>`<button class="chip ${t===activeMapType?'active':''}" data-map-type="${esc(t)}">${esc(t)}</button>`).join('');
  $$('[data-map-type]').forEach(b=>b.onclick=()=>{activeMapType=b.dataset.mapType;renderMaps()});
  const q=$('#mapSearch').value.trim().toLowerCase(), hotOnly=$('#mapHotOnly').checked;
  const visible=map.zones.filter(z=>(activeMapType==='All'||z.type===activeMapType)&&(!hotOnly||z.hot)&&(!q||z.name.toLowerCase().includes(q)||z.type.toLowerCase().includes(q)));
  $('#mapTitle').textContent=map.name; $('#mapSubtitle').textContent=map.subtitle; $('#mapZoneCount').textContent=`${visible.length} of ${map.zones.length} locations`;
  $('#mapEvents').innerHTML=map.events.map(e=>`<span class="detail-tag">${esc(e)}</span>`).join('');
  const stage=$('#mapStage'); stage.style.background=mapThemeArt[map.theme]||mapThemeArt.facility;
  stage.innerHTML=`<div class="map-grid-lines"></div>${visible.map((z,i)=>`<button class="map-marker ${z.hot?'is-hot':''}" style="left:${z.x}%;top:${z.y}%" data-map-zone="${esc(z.name)}" aria-label="${esc(z.name)}, ${esc(z.type)}${z.hot?', hot zone':''}"><span>${i+1}</span><b>${esc(z.name)}</b></button>`).join('')}`;
  $('#mapLocationList').innerHTML=visible.map((z,i)=>`<button class="map-directory-row ${z.hot?'is-hot':''}" data-map-zone="${esc(z.name)}"><span class="map-index">${i+1}</span><span><b>${esc(z.name)}</b><small>${esc(z.type)}</small></span>${z.hot?'<span class="hot-badge">HOT</span>':''}</button>`).join('')||'<div class="empty">No matching map locations.</div>';
  $$('[data-map-zone]').forEach(b=>b.onclick=()=>focusMapZone(b.dataset.mapZone));
}
function focusMapZone(name){
  const map=maps.find(m=>m.id===activeMapId), zone=map?.zones.find(z=>z.name===name); if(!zone)return;
  $$('.map-marker').forEach(x=>x.classList.toggle('selected',x.dataset.mapZone===name));
  $$('.map-directory-row').forEach(x=>x.classList.toggle('selected',x.dataset.mapZone===name));
}

function renderAll(){renderHome();renderItems();renderUpgrades();renderExpeditions();renderBlueprints();renderMaps()}
const benches=[...new Set(blueprints.map(b=>b.bench))].sort();$('#benchFilter').innerHTML+='<option>'+benches.map(b=>`<option value="${esc(b)}">${esc(b)}</option>`).join('');$('#mapSelect').onchange=e=>{activeMapId=e.target.value;activeMapType='All';$('#mapSearch').value='';$('#mapHotOnly').checked=false;renderMaps()};$('#mapSearch').oninput=renderMaps;$('#mapHotOnly').onchange=renderMaps;
